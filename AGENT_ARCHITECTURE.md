...
A general-purpose architecture for building agents and agent suites on a Next.js + Supabase stack. Battle-tested through the Analyst Suite build (7 sub-agents, 20 capabilities, 13+ tables, 14+ API routes). Designed for continuous agent buildout — every new agent strengthens the ecosystem, and every agent can call every other agent.
...
### Pulse (Account Health Monitor)

**Eliminates**: Manual account-by-account health checks. Instead of an account manager maintaining spreadsheets of "who's growing, who's declining, who's at risk," Pulse scans the full book of business automatically and surfaces health signals.

**Accelerates**: Proactive account management. Health scores surface churn risk before it becomes obvious, giving sales teams time to act. The scan/check/history pattern means you get both the broad view and the drill-down.

**Enables for the ecosystem**: A health signal layer on accounts that any downstream agent can consume. Radar can prioritize opportunities in healthy accounts. Playbook can incorporate health context into QBR materials. Brief can pull health trends into executive reports.

**Monitoring loop**: `monitor.history` provides longitudinal health tracking. Trends over time are more valuable than point-in-time snapshots — a declining healthy account is more actionable than a stable mediocre one.

### Radar (Growth Opportunity Scanner)

**Eliminates**: The guesswork of "where should we focus next?" Instead of manual territory analysis and gut-feel prioritization, Radar algorithmically identifies whitespace, expansion potential, and competitive vulnerability across the book of business.

**Accelerates**: Pipeline generation. Opportunities surface from data patterns rather than waiting for reps to notice them. The scan capability produces a ranked, actionable list — not a vague suggestion.

**Enables for the ecosystem**: A structured opportunity layer. Playbook can pull Radar-identified opportunities into territory plans. Pulse health scores can weight opportunity confidence. Brief can compile opportunity summaries for leadership reviews.

### Playbook (Sales Output Compiler)

**Eliminates**: Manual QBR deck building, territory plan assembly, and review preparation. The same data that flows through Pulse and Radar gets compiled into presentation-ready output without copy-paste from multiple sources.

**Accelerates**: Sales meeting prep. A QBR that took hours of data gathering and slide formatting becomes a single capability call. Territory reviews pull from the enriched data layer automatically.

**Enables for the ecosystem**: A sales-specific output engine that complements Brief's analytics output. Where Brief compiles market insights, Playbook compiles sales narratives. Both use the template pattern, both read from the shared data layer, but they serve different audiences and output formats.

**Output versatility**: The three output types — review (account deep-dive), QBR (quarterly business review), territory (territory plan) — cover the core sales presentation formats.
...
- **Suite**: A parent agent with sub-agents. The parent has the dispatch endpoint; sub-agents are logical groupings of capabilities. Example: Analyst Suite (parent) → Rosetta, Sentinel, Pathfinder, Brief, Pulse, Radar, Playbook (sub-agents).
...
|---|---|---|---|
| **Analyst Suite** | `analyst-suite` | (parent — routes to sub-agents) | Orchestrates the full data-to-insight and sales ops pipeline |
...
| **Pulse** | `analyst-pulse` | monitor.scan, monitor.check, monitor.history | Eliminates manual account health tracking; surfaces churn risk proactively |
| **Radar** | `analyst-radar` | opportunity.scan | Eliminates guesswork in growth targeting; algorithmically identifies whitespace and expansion |
| **Playbook** | `analyst-playbook` | playbook.review, playbook.qbr, playbook.territory | Eliminates manual QBR/territory prep; compiles sales-ready output from enriched data |

### Data Flow

```
Raw vendor data (CSV/XLSX drag-and-drop upload)
  ↓
Pipeline Orchestrator (Inngest step function)
  ↓
...
[Brief] compile → reads all enriched data, produces structured output via templates
                    ...
                    │           Sales Ops Pipeline                 │
                    │                                              │
                    │  [Pulse] monitor → account health scores     │
                    │    ↓                                         │
                    │  [Radar] opportunity.scan → ranked opps      │
                    │    ↓                                         │
                    │  [Playbook] compile → QBR, territory, review │
                    └──────────────────────────────────────────────┘
```

Each arrow represents data enrichment, not a direct function call. The agents are decoupled — they communicate through the shared data layer. The dashboard provides a mode switcher (Analytics / Sales Ops) so users see the relevant pipeline for their current workflow.
...
| analyst_templates | User, Agents | Brief, Playbook | Reusable output templates |
| account_health | Pulse | Radar, Playbook, Dashboard | Account health scores and history |
| opportunities | Radar | Playbook, Dashboard | Ranked growth opportunities |
| playbook_outputs | Playbook | Dashboard | Generated QBRs, territory plans, reviews |
| pipeline_runs | Pipeline Orchestrator | Dashboard, All agents | Pipeline execution state and step tracking |
...
| /api/analyst/account-health | GET, POST | Account health scores and monitoring |
| /api/analyst/opportunities | GET, POST | Growth opportunity scanning and listing |
| /api/analyst/playbook | GET, POST | Playbook output generation (QBR, territory, review) |
| /api/analyst/ingest | POST | CSV/XLSX drag-and-drop upload, triggers pipeline |
| /api/analyst/pipeline-status | GET | Pipeline run status and step progress |
